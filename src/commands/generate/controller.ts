...
export default class Generate extends Command {
  static description =
                 ...
                 async run() {
                   ...
                   const entityClass = _.startCase(controller).replace(
                     / /g,
                     ''
                   )
                   const entityIdentity = _.camelCase(controller).replace(
                     / /g,
                     ''
                   )
                   ...
                   if (fs.existsSync(testPath) && !flags.force) {
                     this.warn(
                       `File ${testPath} already exists. Use --force to overwrite.`
                     )
                   } else {
                     renderTemplate(
